...
func (e *ProcessEnv) init() error {
	...
	goEnv, err := e.goCmdEnv(context.TODO())
	if err != nil {
		return err
	}
	if e.Env == nil {
		e.Env = make(map[string]string, len(goEnv))
	}
	for k, v := range goEnv {
		e.Env[k] = v
	}
...
func (p *ProcessEnv) envCacheKey() string {
	workDir := projectDir(p.WorkingDir)

	env := p.env()
	sort.Strings(env)

	n := len(workDir) + len("|") + len(p.BuildFlags) + len("|") + len(env) + 2
	for _, s := range env {
		n += len(s)
	}

	tags := append([]string(nil), p.BuildFlags...)
	sort.Strings(tags)
	for _, s := range tags {
		n += len(s)
	}
...
func (p *ProcessEnv) envCacheKey_X() goEnvCacheKey {
	return goEnvCacheKey{
		WorkDir:    projectDir(p.WorkingDir),
		BuildFlags: strings.Join(p.BuildFlags, "|"),
...
func (p *ProcessEnv) goCmdEnv(ctx context.Context) (map[string]string, error) {
	v, _ := goEnvCache.LoadOrStore(p.envCacheKey(), &goEnvCacheEntry{})
	e := v.(*goEnvCacheEntry)
